...
function CategoryCard({ category }: { category: (typeof categories)[0] }) {
  const { icon: Icon, label, color, bgColor, description, questionCount, topics, userAccuracy, slug } = category;
      ...
      >
        ...
        <div className="flex items-center gap-5 mb-5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-[#6B7280] font-medium">{questionCount.toLocaleString()} questions</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <div className="h-1.5 bg-[#F3F4F6] rounded-full overflow-hidden mb-3">
          <div
            ...
            style={{ width: `${userAccuracy}%`, background: `linear-gradient(90deg, ${color}, ${color}bb)` }}
          />
        </div>

        <div className="flex items-center justify-between">
          <span className="text-xs text-[#6B7280] font-medium">{statusLabel}</span>
          <span className="text-sm font-semibold flex items-center gap-1 hover:gap-2 transition-all" style={{ color }}>
...
function PracticeContent() {
  ...
  return (
    <div className="page-container space-y-8">
      <PageHeader
        title="Practice Hub"
        subtitle="Choose your topic and start practicing"
        actions={
          <Link href="/practice/quiz?category=all&difficulty=all&count=25" className={buttonVariants({ variant: 'primary' })}>
            <Zap size={15} />
            Quick Practice
          </Link>
        }
      ...
      <Card className="p-6">
            ...
            return (
              <button
                ...
                className={`border rounded-xl p-4 flex flex-col items-center gap-1 transition-all duration-150 cursor-pointer ${
                  isActive ? 'border-[#2563EB] bg-[#EFF6FF] shadow-[0_0_0_3px_rgba(37,99,235,0.08)]' : 'border-[#E4E7EC] hover:border-[#2563EB] hover:bg-[#EFF6FF]'
                }`}
              >
                <Ico size={20} className={isActive ? 'text-[#2563EB]' : 'text-[#9CA3AF]'} />
                <span className={`text-lg font-black leading-none ${isActive ? 'text-[#2563EB]' : 'text-[#111827]'}`}>
                  {opt.count}
                </span>
        ...
        <Link
          ...
          className={buttonVariants({ variant: 'primary', className: 'w-full max-w-md' })}
        >
          <Zap size={15} />
          Start Practice
        </Link>
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/analytics" className="flex items-center gap-1.5 text-xs font-semibold text-[#2563EB] hover:text-[#1D4ED8] transition-colors">
            <TrendingUp size={13} />
            View All
          </Link>
        </div>
        ...
        <div className="space-y-3">
          {recentHistory.map((item) => {
            ...
            const scoreColor = ratio >= 0.8 ? '#059669' : ratio >= 0.6 ? '#D97706' : '#DC2626';

            return (
              <Card key={item.topic} hover className="p-5 flex items-center justify-between">
